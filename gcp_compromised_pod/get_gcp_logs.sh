...
(venv) manasbellani@Manass-MacBook-Air /tmp % 
(venv) manasbellani@Manass-MacBook-Air /tmp % 
(venv) manasbellani@Manass-MacBook-Air /tmp % ls
com.apple.launchd.QQ3ECNLt4z  com.apple.launchd.pnElczeStV  drivefs_ipc.501  drivefs_ipc.501_shell  get_gcp_logs.sh  out-gcp-logs.csv  out-gcp-logs.json  ovpnconnect-local-ipc.sock
(venv) manasbellani@Manass-MacBook-Air /tmp % cat get_gcp_logs.sh 
#!/bin/bash
# Summary
#   Script to download the GCP Logs in CSV format with useful fields pre-extracted
# Pre-requisites:
...
HEADER_CSV="timestamp,log_type,log_name,severity,payload_type,client_ip,user_agent,principal_email,service_name,method_name,project_id,resource_name,text_payload,raw"
LOG_TYPE="gcp"
...
for i in $(seq 0 $(($num_logs-1)) ); do
  ...
  text_payload=$(echo "$log" | jq -r ".textPayload")
  raw=$(echo "$log" | tr -s '\"' "'" | tr -d "\n\r")
  echo "\"$timestamp\",\"$LOG_TYPE\",\"$log_name\",\"$severity\",\"$payload_type\",\"$client_ip\",\"$user_agent\",\"$principal_email\",\"$service_name\",\"$method_name\",\"$project_id\",\"$resource_name\",\"$text_payload\",\"$raw\"" >> "$OUTFILE_CSV"
done
